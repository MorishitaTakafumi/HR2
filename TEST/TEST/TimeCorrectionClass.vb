...
Public Class TimeCorrectionClass
    ...
    Public Function createTable() As String
        ...
        Using con As New SQLiteConnection(GetDbConnectionString)
            ...
            Try
                con.Open()
                cmd.CommandText = "SELECT R.class_code, R.type_code, R.kyori, R.jo_code, avg(K.secs) AS ave_time, avg(K.agari) AS ave_agari 
                                    FROM kekka K INNER JOIN raceheader R ON K.race_header_id=R.id 
                                    WHERE K.secs>0 AND K.cyakujun=1 GROUP BY R.class_code, R.type_code, R.kyori, R.jo_code"
    ...
    '時計の補正値取得
    Public Function get_time_correction(ByVal class_code As Integer,
                                        ByVal type_code As Integer,
                                        ByVal kyori As Integer,
                                        ByVal jo_code As Integer) As Single
        Dim ave_times(7) As Single
        Dim std_time As Single = 9999
        For j As Integer = 0 To m_bf.Count - 1
            With m_bf(j)
                If .type_code = type_code AndAlso .kyori = kyori AndAlso .jo_code = jo_code Then
    ...
    '上りの補正値取得
    Public Function get_agari_correction(ByVal class_code As Integer,
                                         ByVal type_code As Integer,
                                         ByVal kyori As Integer,
                                         ByVal jo_code As Integer) As Single
    ...
    '時計の平均値取得
    Public Function get_time_ave(ByVal class_code As Integer,
                                 ByVal type_code As Integer,
                                 ByVal kyori As Integer,
                                 ByVal jo_code As Integer) As Single
        For j As Integer = 0 To m_bf.Count - 1
            With m_bf(j)
                If .class_code = class_code AndAlso .type_code = type_code AndAlso .kyori = kyori AndAlso .jo_code = jo_code Then
                    Return .ave_time
                End If
            End With
        Next
    ...
    '上りの平均値取得
    Public Function get_agari_ave(ByVal class_code As Integer,
                                  ByVal type_code As Integer,
                                  ByVal kyori As Integer,
                                  ByVal jo_code As Integer) As Single
        For j As Integer = 0 To m_bf.Count - 1
